...
int CGame::initGame(void)
{
	...
	if (!m_UI) {
		fprintf(stderr, "Error: CGAME::initGame - Error while creating UI object");
		return RET_ERR;
	}

	iRet = m_UI->init();
	if (RET_ERR == iRet) {
		fprintf(stderr, "Error: CGAME::initGame - Error while initializing UI display");
		return RET_ERR;
	}
	...
	al_register_event_source(m_eventQueue, al_get_display_event_source(m_UI->getDisplay()));
